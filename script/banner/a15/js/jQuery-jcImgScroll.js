		...
		return this.each(function() {
			var _SELF = $(this),
				...
				object = function(length){
					...
					this.arrow = function(l,r,w,h,x,y){
						return "<em class=\"sPrev nopngfilter_spec\" id=\"sPrev\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:"+(l-x)+"px;\"></em><em class=\"sNext nopngfilter_spec\" id=\"sNext\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:"+(r+x)+"px;top:"+y+"px;\"></em>";
					};
			...
			object.prototype.Initial = function(callback){
				...
				var a=null,b=null,c=null,sTmpDD=null,t=2,t2=this.centerCount+1,
					...
					wrapL =parseInt(options.width/4)-50;
					wrapR =wrapL +options.width+50;
				...
				for(a=0;a<this.length;a++){
					...
					if(options.title){
						othis.prepend(this.hoverDom(sTitle,
							 ...
							 .find("b")
							 .css("opacity",0);
							 othis.find("div").append(othis.find("a").clone().removeAttr('title'));
					};
					//获取数字按钮
					if(options.NumBtn){
				...
				for(var b in arrImg){
					...
					arrImg[b].onload = function(){
						...
						$thisImg.html($(arrImg[idx]))
								...
								.end().attr('width',options.width).attr('height',options.height);
					};
				};
				//释放对象
				(function(obj){
